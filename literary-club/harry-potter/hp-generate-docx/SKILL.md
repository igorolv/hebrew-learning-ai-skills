...
2️⃣ Список иллюстраций

Отдельные PNG-файлы страниц, созданные скилом `hp-generate-image`.
Передавать изображения напрямую списком файлов, без ZIP-архива.

Для каждой страницы markdown передать ровно один файл. Порядок файлов во входном
списке не определяет порядок в DOCX: изображения сопоставляются со страницами
по строгим именам, а страницы DOCX следуют в порядке markdown.

Обязательный формат имени:

```text
HP_ch{CHAPTER}_page_{PAGE}.png
```

Пример для главы 1, страниц 36–37:
...
Номер главы и страницы извлекаются только из строгого имени файла.

Алгоритм:

1. Проверить полное имя по шаблону `HP_ch{CHAPTER}_page_{PAGE}.png`.
2. Сопоставить `CHAPTER` с номером главы из имени markdown.
3. Использовать `PAGE` как номер страницы.
4. Отклонить произвольные имена, суффиксы версий и дополнительные числа.

Примеры:

`HP_ch1_page_36.png` → глава 1, страница 36
`HP_ch1_page_37.png` → глава 1, страница 37

Имена `36.png`, `page_36.png`, `HP_ch1_page_36_v2.png` и
`HP_ch2_page_36.png` для markdown главы 1 недопустимы.
...
Преобразовывать поддерживаемые Markdown-маркеры во всех обычных абзацах и
ячейках таблиц:

- `**текст**` → жирный текст без видимых `**`;
- `` `текст` `` → текст с базовой типографикой соответствующего языка и без
  видимых обратных кавычек.

Для жирного иврита задавать bold одновременно как обычное и как complex-script
форматирование (`w:b` и `w:bCs`), сохраняя `David`, 18 pt, RTL и `w:cs`.

Маркеры должны быть парными. При незакрытом `**` или `` ` `` останавливать
генерацию до создания DOCX с ошибкой:

```text
ERROR: invalid inline markdown
```
...
Стандартные «узкие» поля Word (пресет Narrow):
...
Страницы должны присутствовать ровно по одному разу и идти в возрастающем
порядке полного диапазона `FROM…TO`. Перестановка, пропуск, повтор или лишняя
страница вызывают:
...
Имя каждой картинки должно точно соответствовать:

```text
HP_ch{CHAPTER}_page_{PAGE}.png
```

До создания DOCX проверить:

- файл существует и является обычным файлом
- глава картинки совпадает с главой markdown
- для каждой страницы есть ровно одна картинка
- нет картинок страниц за пределами диапазона
- нет дублей
- полный набор страниц присутствует независимо от порядка входного списка
...
ERROR: duplicate image for page N

При несовпадении главы:

ERROR: image belongs to another chapter
