...
Иллюстрации создаются на предыдущем этапе пайплайна с помощью skill'а `hp-generate-image`. Для каждой страницы генерируется промт, по которому в генераторе изображений (ChatGPT / DALL-E и т.п.) создаётся иллюстрация. Готовые изображения собираются в ZIP-архив и подаются на вход этому skill'у.
...
- для ивритского текста в Word нужно задавать `David` не только как обычный шрифт, но и как complex script / bidi font (`cs`), иначе Word может подставить другой шрифт
- если в одной ячейке смешаны иврит и русский, шрифт `David` назначается только ивритскому фрагменту
