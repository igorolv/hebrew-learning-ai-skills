...
Выходной файл — общий вход для скилов `hp-generate-image` и `hp-generate-docx`.
...
### Контракт для hp-generate-image

`hp-generate-image` принимает весь translated-файл и отдельный целевой `PAGE`.
Для надёжного постраничного извлечения:

- блоки `# Страница N` должны идти ровно в порядке `FROM…TO`, без пропусков и повторов
- каждая страница должна содержать по одной секции `Иврит`, `Подстрочный перевод`,
  `Литературный перевод` и `Различия ивритского и русского переводов`
- `Иврит` и полный подстрочник должны описывать один и тот же фрагмент страницы
- литературный перевод не должен смешиваться с соседними страницами
- различия переводов должны отмечать добавления Росмэна, способные изменить визуальную сцену

Номер главы для имени изображения извлекается из имени translated-файла,
а номер страницы — из целевого блока. Результат `hp-generate-image` называется
...
Детальные правила («когда ставить» / «когда не ставить» / технические принципы / обязательный аудит / частые ошибки) вынесены в отдельный файл **`references/nikud-rules.md`**. Перед расстановкой никуда **прочитать его целиком**. Раздел «Обязательный постраничный аудит» — ворота качества, а не рекомендация.

### 3.3 Краткое резюме

**Никуд ставится на:**
1. Все лексемы из таблицы «Сложные слова» входного файла — **каждое вхождение и все их формы**, а не только точное совпадение со словарной формой.
...
### 3.4 Обязательный аудит покрытия никудом

Для каждой страницы сначала составить рабочую карту всех ивритских слов, и только потом записывать секцию «Иврит». Карта временная и в итоговый markdown не попадает.

1. Пройти страницу слово за словом и присвоить каждому слову ровно один статус: `огласовать`, `не огласовывать`, `проверить`.
2. Выполнить четыре независимых прохода:
   - **таблица:** найти каждое вхождение каждой лексемы из «Сложных слов», включая формы с приставками, суффиксами, другим числом, родом, лицом и временем; не путать их с омографами, у которых совпадают буквы, но отличаются значение и никуд;
   - **имена:** найти все вхождения имён, географических названий и заимствований;
   - **морфология:** определить биньян и форму **каждого глагола**, затем огласовать все формы, обязательные по `nikud-rules.md`;
   - **лексика:** проверить все оставшиеся содержательные слова на редкость, книжность, неоднозначность чтения и трудность для уровня алеф–бет.
3. Разрешить все статусы `проверить`: если нет уверенности, что ученик прочитает слово с первого взгляда, перевести его в `огласовать`.
4. Полностью огласовать каждое выбранное слово. Нельзя ограничиваться одним знаком-подсказкой. Если точный полный никуд неизвестен, проверить форму по надёжному словарю или морфологическому источнику; не оставлять трудное слово голым ради обхода проверки и не угадывать знаки.
5. Не переходить к подстрочнику, пока одновременно не выполнены условия:
   - обязательных, но неогласованных слов нет;
   - статусов `проверить` не осталось;
   - после удаления всех знаков никуда последовательность базовых букв, слов и пунктуации секции «Иврит» совпадает со входным текстом;
   - плотность визуально сопоставлена с эталоном `HP_ch3_11_13_translate.md`. Явно более редкий никуд требует повторить четыре прохода, но не является поводом огласовывать простые слова ради процента.

Уже имеющийся во входе частичный никуд не заменяет аудит: классифицировать нужно **каждое** слово страницы.
...
**Шаг 3.** Создать секцию «Иврит»:
   - Взять ивритский текст из входного файла, не меняя базовые буквы, слова, повторы и пунктуацию
   - Выполнить обязательный аудит из раздела 3.4 и раздела «Обязательный постраничный аудит» в `references/nikud-rules.md`
   - Для каждой лексемы из таблицы сложных слов найти **все её формы и все вхождения**; не ограничиваться поиском точной строки из ячейки
   - Определить биньян и форму каждого глагола, а не только тех, которые сразу выглядят сложными
   - Полностью огласовать все слова со статусом `огласовать`, включая каждое появление имён и названий
   - **Не удалять и не заменять «матери чтения»** — сохранять полное написание
   - Сохранить жирный шрифт (`**...**`) как в оригинале
   - Удалить никуд мысленно из результата и пословно сверить с входом; только после успешной сверки переходить к шагу 4
...
- [ ] После удаления никуда базовые буквы, слова, повторы и пунктуация секции «Иврит» буквально совпадают со входным текстом; «матери чтения» не удалены и не заменены
- [ ] Простые слова (אני, של, את и т.д.) НЕ огласованы
- [ ] Для каждой страницы построена полная рабочая карта слов; статусов `проверить` и обязательных неогласованных слов не осталось
- [ ] Каждая лексема из таблицы сложных слов найдена во всех формах и огласована при каждом вхождении
- [ ] Имена собственные, географические названия и заимствования огласованы при каждом вхождении
- [ ] Каждый глагол классифицирован по биньяну и форме; все обязательные по `nikud-rules.md` формы огласованы
- [ ] Все оставшиеся содержательные слова проверены на редкость, книжность, омографию и неочевидное чтение для уровня алеф–бет
- [ ] Плотность никуда сопоставлена с эталоном; явно более редкий результат прошёл повторный аудит, при этом простые слова не добавлены ради процента
...
- [ ] Блоки `# Страница N` идут ровно в порядке `FROM…TO`, без пропусков и повторов
- [ ] На каждой странице присутствуют все секции, обязательные для `hp-generate-image`

В финальном сообщении привести компактный отчёт **для каждой страницы**: `формы таблицы X/X; имена и названия Y/Y; обязательные глагольные формы Z/Z; проверить 0; базовый текст OK`. Эти числа — результат рабочей карты, а не оценка на глаз. Если какой-либо счётчик не закрыт, файл не выдавать как готовый.
...
3. **Поиск только точного слова из таблицы** — нужно находить каждое вхождение той же лексемы во всех формах. Например, `לְהָצִיץ` требует огласовать и `הֵצִיץ`, и `הֵצִיצוּ`; `מַעֲבָר` — и `בַּמַּעֲבָר`, и `בַּמַּעֲבָרִים`.

4. **Один быстрый проход по “заметно сложным” словам** — приводит к пропуску биньянов и редкой лексики. Обязательны все четыре прохода из раздела 3.4.

5. **Добавление слов в таблицу сложных слов**, которых нет во входном файле.

6. **Свободный пересказ** вместо перевода Росмэн в литературном переводе.

7. **Слишком крупные фрагменты** в подстрочном переводе — лучше разбивать на фразы по 5–15 слов.

8. **Пропуск огласовок** на именах собственных — имена должны быть всегда огласованы.

9. **Неполный подстрочник** — самая болезненная ошибка для пользователя, потому что он читает главу именно по подстрочнику. Типичные пропуски, которые нужно ловить:
...
10. **Дрейф к ритму Росмэна** в подстрочнике — Росмэн сливает, опускает и перестраивает; подстрочник следует иврит**у**. Если при составлении таблицы ловишь себя на мысли «но ведь у Росмэна этого нет» — это сигнал, что ты сбился: подстрочник строится от ивритского текста.
