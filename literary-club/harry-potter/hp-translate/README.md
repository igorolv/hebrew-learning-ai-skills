# hp-translate

Claude Skill для добавления селективных огласовок (никуд), подстрочного перевода, литературного перевода (Росмэн) и анализа различий к ивритскому тексту Гарри Поттера.

## Что делает

- Добавляет селективные огласовки (никуд) с учётом уровня ученика (алеф–бет)
- Перед подстрочником проводит обязательный постраничный аудит каждого слова: формы лексем из таблицы, имена, биньяны и оставшаяся сложная лексика
- Проверяет, что обязательных неогласованных и неразрешённых слов не осталось, а базовый иврит после удаления никуда совпадает со входом
- В финальном сообщении показывает постраничные счётчики покрытия форм из таблицы, имён и обязательных глагольных форм
- Создаёт подстрочный (дословный) перевод в таблице
- Добавляет литературный перевод по канону издательства Росмэн
- Анализирует значимые различия между ивритским и русским переводами
- Сохраняет «матери чтения» (ктив мале) при добавлении огласовок
- Создаёт единый translated markdown для `hp-generate-image` и `hp-generate-docx`

## Как использовать

### В Claude.ai (Projects)
Добавьте файл `SKILL.md` в Project Knowledge вашего проекта.

### В Claude Code
Скопируйте папку в директорию skills и подключите через конфигурацию.

## Структура

```
hp-translate/
├── SKILL.md                — основная инструкция
├── README.md
└── references/
    ├── nikud-rules.md               — полные правила и обязательный аудит никуда
    └── HP_ch3_11_13_translate.md    — эталон плотности и формата результата
```

## Входные данные

Markdown-файл, созданный skill'ом `hp-extraction` (например, `HP_ch1_30_35.md`).

## Выходные данные

`HP_ch{CHAPTER}_{FROM}_{TO}_translate.md` с последовательными блоками
`# Страница N`. Файл передаётся:

- в `hp-generate-image` вместе с целевым номером страницы
- в `hp-generate-docx` вместе со списком готовых PNG

### Зависимости

Для секций «Литературный перевод» и «Различия переводов» требуются исходные тексты в Project Knowledge:
- `HP_en_ch{N}.md` — английский оригинал главы
- `HP_rosmen_ch{N}.md` — русский перевод Росмэн

Поиск этих файлов описан в skill'е [hp-source-texts](../hp-source-texts).

## Часть проекта

Этот skill — второй этап в пайплайне обработки ивритского Гарри Поттера:

1. [hp-extraction](../hp-extraction) — извлечение текста из PDF
2. **hp-translate** ← вы здесь
   - зависимость: [hp-source-texts](../hp-source-texts) — поиск английского и русского текстов
3. [hp-generate-image](../hp-generate-image) — одна PNG-иллюстрация на целевую страницу
4. [hp-generate-docx](../hp-generate-docx) — сборка translated markdown и PNG в DOCX
