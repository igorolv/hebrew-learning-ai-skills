...
description: >
  ...
  «enrich the lesson», а также по явному подтверждению следующего шага после извлечения.
  На вход скилл принимает *извлечённый* markdown — НЕ исходный PPTX.
  Запускать, когда пользователь передаёт директорию урок_*_extracted/ или extracted Markdown и просит обработать
  его дальше, либо когда скилл вызван оркестратором `hebrew-homework-pipeline`.
  После одиночного извлечения только предложить обогащение; не начинать его без запроса.
---
...
- Принимать extracted Markdown и соседнюю `images/`, а не исходный PPTX.
- Не выполнять упражнения и не создавать DOCX.
- Не запускаться автоматически после одиночного `hebrew-lesson-extract`; дождаться запроса либо
  явного вызова от `hebrew-homework-pipeline`.
- Создавать результат в рабочей директории пользователя, а не в каталоге скилла.
...
1. **Извлечённый материал урока** — выход скила `hebrew-lesson-extract`
   - Директория `урок_N_часть_M_extracted/`:
     ...
     - `images/` — извлечённые изображения (для обогащения обычно не используются)
   - Если скил `hebrew-lesson-extract` ещё не выполнялся — предложить пользователю сначала запустить его на PPTX-файле урока
   - **Где искать**: во вложениях текущей задачи, рабочей директории пользователя и доступных
     подключённых источниках знаний
...
2. **Прочитать предыдущие шпаргалки** — если обрабатывается часть 2+, найти во вложениях,
   рабочей директории или доступных подключённых источниках знаний уже готовые части.
   Это критически важно для кумулятивных таблиц глаголов и единообразия
...
Использовать доступный поиск по рабочей директории и подключённым источникам знаний.
Не предполагать наличие конкретного connector или инструмента.
...
Разместить её в рабочей директории пользователя и вернуть абсолютную ссылку средствами
текущей среды.
