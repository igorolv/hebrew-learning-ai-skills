...
description: >
  ...
  домашнего задания, создать DOCX из результата скила hebrew-homework, собрать задание в Word,
...
Используется в пайплайне подготовки домашних заданий, после скила `hebrew-homework`.

---

# Входные данные

Skill принимает один или несколько markdown-файлов — результат работы скила `hebrew-homework`.

Формат имени:

ДЗ_урок_N_часть_M_слайды_X_Y.md
...
Markdown от скила `hebrew-homework` имеет следующую структуру:
...
Поведение: вставить spacing (360 twips before) перед следующим параграфом. НЕ вставлять разрыв страницы — домашнее задание компактнее книги и не требует разрывов на каждом слайде.
...
Для каждого run с ивритом обязательно задавать:
- `w:rFonts` с `w:cs = "David"`
- `w:rtl`
- `w:cs` (complexScript)
- `w:sz` и `w:szCs`
...
Разрывы страниц НЕ вставляются автоматически между слайдами.

Разрыв страницы вставляется только если пользователь явно запросит.
